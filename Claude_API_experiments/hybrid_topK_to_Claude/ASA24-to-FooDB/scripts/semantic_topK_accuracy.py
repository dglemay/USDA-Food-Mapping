...
def load_inputs():
    inputdf = pd.read_csv(input_file, sep="\t", header=0) #header specifies which row contains the header
    # turning the food descriptions in each column to lowercase
    inputdf['input_desc'] = inputdf['input_desc'].str.lower()

    targetdf = pd.read_csv(target_file, sep="\t", header=0) 
    # turning the food descriptions in each column to lowercase
...
def load_truth():
    df = pd.read_csv(truth_file, sep="\t", header=0)
...
if __name__ == "__main__":
    ...
    for K in K_list:
        ...
        print(f"Top {K} accuracy is {topK_match_accuracy}")
